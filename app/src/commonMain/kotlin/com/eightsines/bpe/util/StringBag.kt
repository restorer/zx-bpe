...
private const val BAG_SIG = "BAG1"
...
class PackableStringBag : PackableBag {
    private val buffer = StringBuilder(BAG_SIG)
...
class UnpackableStringBag(private val input: String) : UnpackableBag {
    private val endIndex = input.length - 1
    private var lastIndex: Int = BAG_SIG.length - 1

    init {
        if (!input.startsWith(BAG_SIG)) {
            throw BagUnpackException("Missing \"$BAG_SIG\" signature")
        }
    }
