...
class GraphicsEngine(
    ...
    fun canExecute(action: GraphicsAction): Boolean = when (action) {
        ...
        is GraphicsAction.SetLayerMasked -> canSetLayerMasked(action) != null
    ...
    fun execute(action: GraphicsAction): GraphicsAction? {
        ...
        val undoAction = when (action) {
            ...
            is GraphicsAction.SetLayerMasked -> executeSetLayerMasked(action)
    ...
    private fun executeReplaceLayer(action: GraphicsAction.ReplaceLayer): GraphicsAction? {
        ...
        val layer = (action.layer as MutableCanvasLayer<*>).copyMutable()
    ...
    private fun canSetLayerMasked(action: GraphicsAction.SetLayerMasked): MutableCanvasLayer<*>? {
        val layer = canvasLayersMap[action.layerUid.value] ?: return null
        return if (layer.isMasked == action.isMasked) null else layer
    }

    private fun executeSetLayerMasked(action: GraphicsAction.SetLayerMasked): GraphicsAction? {
        val layer = canSetLayerMasked(action) ?: return null

        val undoAction = GraphicsAction.SetLayerMasked(layer.uid, layer.isMasked)
        layer.isMasked = action.isMasked
        return undoAction
    }
    ...
    private fun executeMergeLayers(action: GraphicsAction.MergeLayers): GraphicsAction? {
        ...
        mergeCanvas.mutate { mutator ->
            walkInBox(mergeWidth, mergeHeight) { x, y ->
                mutator.replaceSciiCell(
                    x,
                    y,
                    Merger.mergeSciiCell(canvas.getSciiCell(x, y), mergeCanvas.getSciiCell(x, y)),
                )
            }
        }
    ...
    private fun executeUndoMergeLayers(action: GraphicsAction.UndoMergeLayers): GraphicsAction? {
        ...
        val replaceLayer = (action.replaceLayer as MutableCanvasLayer<*>).copyMutable()
        canvasLayers[getLayerIndex(replaceLayer.uid)] = replaceLayer
        canvasLayersMap[replaceLayer.uid.value] = replaceLayer

        val insertLayer = (action.insertLayer as MutableCanvasLayer<*>).copyMutable()
